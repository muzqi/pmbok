...
// 项目背景+过渡段落模版
const projectBackgroundRender = (title, isSummary = false) => {
  const background = `
一、项目概述
...
区别于2C用户，为小微企业提供800万像素，具备GPS、夜视等功能的高清抓拍枪机，并提供国标GB28181协议接入的硬件服务。

项目总投资3000万，工期21个月，要求交付源代码、操作手册、产品文档、视频监控系统（WEB + APP）、摄像机管理系统、开放平台接入系统。
我被任命为项目经理，负责该项目的管理和实施。
项目采用项目型组织结构${isSummary ? '' : '，其中需求分析师2人，系统设计师4人，系统开发工程师14人，系统测试人员4人，质量保证人员2人，系统运维人员2人，共计28人'}。
  `;

  const transition = `
该项目需要将流媒体服务和AI人脸分析系统深度融合，技术复杂，实现难度大。
又需要抹平不同厂商、型号摄像机的私有协议，梳理和整合的难度大，稍有疏忽就会导致项目的失败。
...
在视频智能监控小微平台系统规划阶段，我依据${basis.map((d, i) => {
  ...
  return s;
}).join('')}等资料，邀请总监理师王工、部门领导、团队关键干系人，以专题会议的形式对平台建设的管理、组织结构、审批流程、${title}绩效审计周期，${title}责任矩阵等内容进行了深入探讨。
...
2022年3月该项目成功通过我司高层验收，不仅进度提前11天而且费用也节约了20万，系统也实现了我司布局2B小微企业细分领域的目标，
系统运行至今稳定可靠，受到了领导和用户的一致好评，项目取得了圆满成功。
项目的成功首先来自全体团队成员的不懈努力，其次得益于我科学高效的${title}以及相关工具和技术的运用。
但管理上也存在一些不足，如：由于我们对新员工的培训不足，新员工对业务和质量规范不熟悉，需求理解与干系人的期望有偏差，导致部分模块返工；
